...
namespace attendanceSystem.userControls
{
    public partial class Attendace : UserControl
    {
        ...
        private void dataGridViewStudentAtt_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            // Check if the edited cell is in the Status column
            if (e.ColumnIndex == dataGridViewStudentAtt.Columns["Status"].Index && e.RowIndex >= 0)
            {
                // Get the edited value
                string newStatus = dataGridViewStudentAtt.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString();

                // Get the corresponding student's ID
                string studentId = dataGridViewStudentAtt.Rows[e.RowIndex].Cells["RegNo"].Value.ToString();
                ...
                // Find the corresponding user node in the XML
                XmlNode userNode = xmlDoc.SelectSingleNode($"//user[Id='{studentId}']");

                // Find the corresponding attendance record for the selected date
                XmlNode attendanceNode = userNode.SelectSingleNode($"Attendance/Record[Date='{selectedDate}']");

                if (attendanceNode != null)
                {
                    // Update the status in the XML
                    attendanceNode.SelectSingleNode("Status").InnerText = newStatus;
                }
                else
                {
                    ...
                    XmlElement newDate = xmlDoc.CreateElement("Date");
                    newDate.InnerText = selectedDate;
                    XmlElement newStatusElement = xmlDoc.CreateElement("Status");
                    newStatusElement.InnerText = newStatus;
                    newRecord.AppendChild(newDate);
                    newRecord.AppendChild(newStatusElement);

                    // Check if Attendance node exists, if not create it
                    XmlNode userAttendanceNode = userNode.SelectSingleNode("Attendance");
                    if (userAttendanceNode == null)
                    {
                        userAttendanceNode = xmlDoc.CreateElement("Attendance");
                        userNode.AppendChild(userAttendanceNode);
                    }

                    userAttendanceNode.AppendChild(newRecord);
                }
